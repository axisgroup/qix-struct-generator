...
/***/ function(module, exports, __webpack_require__) {
	...
	var _structs = __webpack_require__(299);

	var _structs2 = _interopRequireDefault(_structs);
	...
	var structs = JSON.parse(_structs2.default);
	var structToBuild = "HyperCubeDef";
	var myLayout = buildLayout(structToBuild, structs);

	var updateProperty = (0, _util.curry)(function (layout, property, value, propAccessor) {
	    var prop = getValue(layout, propAccessor);
	    prop[property] = value;
	});

	var cAddProperty = (0, _util.curry)(addProperty);
	var addLayoutProp = cAddProperty(structs, myLayout);
	var collapseProp = updateProperty(myLayout, "collapsed", true);
	var expandProp = updateProperty(myLayout, "collapsed", false);
	var enableProp = updateProperty(myLayout, "enabled", true);
	var disableProp = updateProperty(myLayout, "enabled", false);

	render(myLayout);

	window.structs = structs;
	window.myLayout = myLayout;
	window.curry = _util.curry;
	window.addProperty = addProperty;
	window.addLayoutProp = addLayoutProp;
	window.collapseProp = collapseProp;
	window.expandProp = expandProp;
	window.enableProp = enableProp;
	window.disableProp = disableProp;

	function buildLayout(structToBuild, structs) {
	    return structs.filter(function (f) {
	        return f.struct === structToBuild;
	    }).reduce(function (acc, curr) {
	        var copied = Object.keys(curr).reduce(function (cAcc, cCurr) {
	        ...
	        if (copied.array) {
	            ...
	            copied.value = buildLayout(copied.type, structs);
	        } else {
	...
	function render(layout) {
	    ...
	    var html = Object.keys(layout).reduce(function (acc, curr) {
	        var row = "<div class='row'>";
	        row += "<div class='" + (layout[curr].enabled ? "enabled" : "disabled") + "'>" + curr + "</div>";
	        row += "<div class='onOff'>toggle on/of</div>" + "<div class='colExp'>collapse/expand</div>";
	        row += "</div>";
	        return acc + row;
	    }, "");
	    list.innerHTML = html;
	...
	function addProperty(structs, layout, propAccessor) {
	    var prop = getValue(layout, propAccessor);
	    var value = void 0;
	    if (["Boolean", "Integer", "String"].indexOf(prop.type) > -1) {
	        value = prop.default;
	    } else {
	        value = buildLayout(prop.type, structs);
	    }
	...
	function getValue(struct, propAccessor) {
	    var accessors = propAccessor.split("/");
	    return accessors.reduce(function (acc, curr) {
	        return acc[curr];
	    }, struct);
	}
...
/***/ function(module, exports) {
	...
	var curry = function curry(f) {
	    ...
	    if (f.length <= args.length) return f.apply(undefined, args);
	    return function () {
